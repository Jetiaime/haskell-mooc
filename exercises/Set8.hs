...
blendColor (Color ar ag ab) (Color br bg bb) = Color ((ar + br) `div` 2) ((ag + bg) `div` 2) ((ab + bb) `div` 2)

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture p1) (Picture p2) = let f1 c = f (p1 c) (p2 c) in Picture f1    
...
union (Shape f1) (Shape f2) = let f c = f1 c || f2 c in Shape f

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = let f c = f1 c && not (f2 c) in Shape f
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) (Picture p) = Picture f
    where f c = color
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where f (Coord x y) = let neighbors = [Coord x y, Coord (x-1) y, Coord (x+1) y, Coord x (y-1), Coord x (y+1)]
                              neighborColors = map p neighbors
                              rs = [getRed c | c <- neighborColors]
                              gs = [getGreen c | c <- neighborColors]
                              bs = [getBlue c | c <- neighborColors]
                          in Color (sum rs `div` length rs) (sum gs `div` length gs) (sum bs `div` length bs)
...
instance Transform BlurMany where
  apply (BlurMany n) p
    | n == 0 = p
    | otherwise = apply Blur $ apply (BlurMany (n - 1)) p
------------------------------------------------------------------------------
