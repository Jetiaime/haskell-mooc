...
greetText a
    | T.length a <= 15 = T.pack "Hello, " <> a <> T.pack "!"
    | otherwise = T.pack "Hello, " <> T.take 15 a <> T.pack "...!"
...
shout :: T.Text -> T.Text
shout xs = T.unwords $ [if even i then T.toUpper word else word | (i, word) <- zip [0..] (T.words xs)]
...
longestRepeat a = go xs 0 0
        where
            xs = T.unpack a
            go "" mx cur = max mx cur
            go [x] mx cur = max mx (cur + 1)
            go (x : y : ss) mx cur
              = if x == y then
                    go (y : ss) mx (cur + 1)
                else
                    go (y : ss) (max mx (cur + 1)) 0
...
pingpong obs = BL.pack $ cycle (bs ++ (reverse bs))
    where bs = B.unpack obs
