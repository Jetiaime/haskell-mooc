...
add v (Set (x:xs))
  | v < x     = Set (v : x : xs)
  | v == x    = Set (x : xs)
  | otherwise = case add v (Set xs) of Set ys -> Set (x : ys)
...
data State = Start | HaveEggs | FlourAdded | SugarAdded | AddAll | Mixed | Finished | Error
  deriving (Eq,Show)
...
step HaveEggs AddFlour = FlourAdded
step HaveEggs AddSugar = SugarAdded
step FlourAdded AddSugar = AddAll
step SugarAdded AddFlour = AddAll
step AddAll Mix = Mixed
...
instance (Ord a, Eq a) => Semigroup (Set a) where
  Set [] <> set2 = set2
  set1 <> Set [] = set1
  Set (x:xs) <> set2 = add x (Set xs <> set2)
...
passwordAllowed s (MinimumLength l) = l <= length s
...
data Arithmetic = Literal Integer
                | SumOperation Arithmetic Arithmetic
                | ProductOperation Arithmetic Arithmetic
  deriving Show
...
operation s a b = case s of "+" -> SumOperation a b
                            "*" -> ProductOperation a b

evaluate :: Arithmetic -> Integer
evaluate (Literal n) = n
evaluate (SumOperation a b) = evaluate a + evaluate b
evaluate (ProductOperation a b) = evaluate a * evaluate b

render :: Arithmetic -> String
render (Literal n) = show n
render (SumOperation a b) = "(" ++ render a ++ "+" ++ render b ++ ")"
render (ProductOperation a b) = "(" ++ render a ++ "*" ++ render b ++ ")"
