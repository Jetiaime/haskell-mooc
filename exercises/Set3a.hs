...
capitalize s = unwords [capitalized | word <- words s, let capitalized = capitalizeFirst word]
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) [x | i <- [0..max], let x = k^i]
...
joinToLength l s = [i ++ j | i <- s, j <-s, (length i) + (length j) == l ]
...
sumRights x = go 0 x
    where go sum [] = sum
          go sum (x:xs) = case x of 
            (Right rx) -> go (sum + rx) xs
            (Left lx) -> go sum xs
...
multiCompose fs x = go x id fs
    where go x q [] = q x
          go x q (f:fs) = go x (q . f) fs
...
interpreter commands = reverse $ go commands 0 0 []
    where go [] x y r = r
          go (c:cs) x y r = case c of "printX" -> go cs x y (show x:r)
                                      "printY" -> go cs x y (show y:r)
                                      "up" -> go cs x (y + 1) r
                                      "down" -> go cs x (y - 1) r
                                      "left" -> go cs (x - 1) y r
                                      "right" -> go cs (x + 1) y r
