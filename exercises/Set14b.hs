...
balance conn account = do
  rows <- query conn balanceQuery [account] :: IO [[Int]]
...
parseCommand' :: T.Text -> Maybe String
parseCommand' = Just . T.unpack

parseCommand :: [T.Text] -> Maybe Command
parseCommand [command, account] = case parseCommand' command of
    Just "balance" -> Just (Balance account)
    Just _ -> Nothing
parseCommand [command, account, amountText] = case parseCommand' command of
    Just "deposit" -> case parseInt amountText of
      Just amount -> Just (Deposit account amount)
      Nothing -> Nothing
    Just "withdraw" -> case parseInt amountText of
        Just amount -> Just (Withdraw account amount)
        Nothing -> Nothing
    Just _ -> Nothing
...
perform conn (Just (Balance account)) = do
  balanceNum <- balance conn account
  return $ T.pack $ show balanceNum
...
server db request respond = do
  ...
  let command = parseCommand paths
