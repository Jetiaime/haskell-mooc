...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = let r = if i == len then len else i + 1
                      len = length xs 
                  in take i xs ++ [x] ++ drop r xs
...
isPalindrome str = (take mid str) == reverse (takeFinal mid str)
...
palindromify s = if l == 0
    then ""
    else if isPalindrome s
        then s
        else palindromify (substring 1 (l - 1) s)
    where l = length s
...
greet first last = "Hello, " ++ first ++ safeLastName last ++ "!"

safeLastName :: Maybe String -> String
safeLastName Nothing = ""
safeLastName (Just last) = " " ++ last
