...
rgb (Invert g) = map (1 -) (rgb g)
rgb (Mix g1 g2) = zipWith (\x y -> (x + y) / 2) (rgb g1) (rgb g2)
...
fromList [] = Empty
fromList ((k,v):ps) = Pair k v (fromList ps)
...
toNat z
  | z < 0 = Nothing
  | z == 0 = Just Zero
  | otherwise = case toNat (z - 1) of Just g -> Just (PlusOne g)
...
toBin 0 = O End
toBin 1 = I End
toBin x = if x `mod` 2 == 1
          then I (toBin (x `div` 2))
          else O (toBin (x `div` 2))
