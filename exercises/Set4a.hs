...
distinct a = (length a) == (length (nub a))
...
longest [] = []
longest [x] = x
longest (x:xs) = case comparing length x y
                 of EQ -> min x y
                    LT -> y
                    GT -> x
                 where y = longest xs
...
incrementKey a [] = []
incrementKey a ((k, v):xs) = (if a == k then (k, v + 1) else (k, v)) : incrementKey a xs
...
winner scores player1 player2 = let score1 = Map.findWithDefault 0 player1 scores
...
maxIndex a = fst $ foldr (\(i, e) (maxI, maxE) -> if e > maxE then (i, e) else (maxI, maxE)) (head pairs) pairs
             where pairs = Data.Array.assocs a
