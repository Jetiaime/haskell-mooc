...
isOrdered (Node val l r) = isOrdered l && isOrdered r && allValues (< val) l && allValues (> val) r
...
walk (StepL:steps) Empty = Nothing
walk (StepR:steps) Empty = Nothing
...
search val (Node v l r)
  | v == val = Just []
  | otherwise = case search val l of Nothing -> case search val r of Just x -> Just (StepR: x)
                                                                     Nothing -> Nothing
                                     Just x -> Just (StepL:x)
