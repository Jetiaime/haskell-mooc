...
countValid = length . filter (\(_:_:a:b:c:e:s) -> a == c || b == e)
...
sumSuccess (x:xs) = case x of (Left _) -> sumSuccess xs
                              (Right x) -> case sumSuccess xs of Right y -> Right (x + y)
                                                                 Left y -> Right x
...
open s (Opened ans) = Opened ans
open s (Locked ans) = if s == ans then Opened s else Locked ans

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Opened a) = Locked a
lock l = l

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode s (Opened a) = Opened s
changeCode s l = l
...
instance Eq Text where
  (==) (Text "") (Text "") = True
  (==) (Text "") (Text s2) = all Data.Char.isSpace s2
  (==) (Text s1) (Text "") = all Data.Char.isSpace s1
  (==) (Text (s1 : xs1)) (Text (s2 : xs2))
    | not (Data.Char.isSpace s1) && not (Data.Char.isSpace s2) = s1 == s2 && Text xs1 == Text xs2
    | Data.Char.isSpace s1 = Text xs1 == Text (s2 : xs2)
    | otherwise = Text (s1 : xs1) == Text xs2
...
compose [] _ = []
compose (a:xs) ys = case lookup (snd a) ys of Nothing -> compose xs ys
                                              Just t -> (fst a, t) : compose xs ys
...
permute p l = map snd $ sortBy (\(i1, _) (i2, _) -> compare i1 i2) $ compose [(p !! i, i) | i <- idl] [(i, l !! i) | i <- idl]
              where idl = identity (length p)
